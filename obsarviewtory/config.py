...
class VolcanoArea:
    """Stores needed information for locating a volcano area and processing it
    ...
    filter_dates: list
        Dates to be ignored when processing this volcano
    ul : int
        The upper-left corner of the area for this volcano,
        in UTM coordinates
    lr : int
        The lower-right corner of the area for this volcano,
        in UTM coordinates
    path : int
        The satellite Path covering this volcano
    frame: int
        The satellite frame covering this volcano
    asf_name : str
        The name of the ASF project covering this volcano
    run_flag : bool
        A flag indicating if this volcano should be included
        in the processing workflow

    Methods
    -------
    update_flag(new_flag)
        Updates the update flag with the new value
    """
    ...
    def __init__(self, volc_name, ul, lr, path, frame, asf_name, run_flag=0):
        ...
        # if aoi contains >1 volcanoes, label volc_name would only be one of them
        ...
        # bad dates to skip (e.g. shifted-frame scenes)
        self.filter_dates = filter_dates.get((volc_name, path), [])

        # [x,y] aoi information, satellite coordinates of upper-left conner
        self.ul = ul
        self.lr = lr  # lower-right
        self.path = path  # integer, same for frame
        self.frame = frame
        # asf vertex project name, string, must be same as that shown on asf
        self.asf_name = asf_name
        # indicate whether to run or skip the processing of current volcano; 1 for run
...
kiska30 = VolcanoArea(
    'kiska', [513500, 5776800], [548600, 5741000], 30, 420, 'st_semisopochnoi_real'
)
davidof30 = VolcanoArea(
    'davidof', [574600, 5768000], [609300, 5750700], 30, 420, 'st_semisopochnoi_real'
)
semisopochnoi30 = VolcanoArea(
    'semisopochnoi', [669600, 5767700], [690700, 5750000], 30, 420, 'st_semisopochnoi_real'
)

makushin41044 = VolcanoArea(
    'makushin410', [355800, 5987000], [428600, 5930000], 44, 410, '044_410'
)
...
makushin41544 = VolcanoArea(
    'makushin415', [309000, 5976000], [426800, 5903000], 44, 415, '044_415'
)
...
takawangha59 = VolcanoArea(
    'takawangha', [415000, 5753200], [457400, 5716100], 59, 420, '059_419'
)
...
greatsitkin59 = VolcanoArea(
    'greatsitkin', [554000, 5774600], [570000, 5757000], 59, 420, '059_419'
)
...
semisopochnoi81 = VolcanoArea(
    'semisopochnoi', [669700, 5767500], [690600, 5750000], 81, 166, '081_166'
)
...
aniakchak102 = VolcanoArea(
    'aniakchak', [500000, 6357000], [659000, 6250000], 102, 402, '102_402'
)
...
cleveland117 = VolcanoArea(
    'cleveland', [555000, 5882600], [589900, 5841000], 117, 417, '117_417'
)
...
augustine131 = VolcanoArea(
    'augustine', [467000, 6587000], [480800, 6575500], 131, 393, '131_393'
)
...
greatsitkin161 = VolcanoArea(
    'greatsitkin', [553800, 5774700], [570700, 5757700], 161, 418, '161_418'
)
atka161 = VolcanoArea('atka', [600600, 5812000], [705200, 5763000], 161, 418, '161_418')

veniaminof102 = VolcanoArea(
    'veniaminof', [467500, 6253000], [526900, 6203000], 102, 407, '102_407'
)
