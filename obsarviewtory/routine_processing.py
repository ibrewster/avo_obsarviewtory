...
class InSARProcessor:
    def __init__(self):
        ...
        self.processing_dir = Path.cwd() / 'processings'
        
        # Directory to store the output InSAR products in
        self.product_dir = Path(os.path.dirname(__file__),  'products' , 'insar')
        ...
        try:
            ...
            for product in ('timeseries_ERA5_demErr', 'velocity'):
                ...
                shutil.move(str(mintpy_directory / f"{product}.h5"),
                            str(self.product_dir / f"{dest}_{output_ident}") )
...
if __name__ == "__main__":
    ...
    print(f"Run complete in {runtime} {units}")
